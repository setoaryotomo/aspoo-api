...
@dataclass
class Item:
    """Optimized Item class for customer recommendations"""
    id: int
    category: str
    name: str
    price: float
    ...
    value: float
    store: str = ""
    value_weight_ratio: float = 0.0
    value_price_ratio: float = 0.0
...
# Utility functions for data loading and API integration

def load_items_from_excel(excel_path: str) -> List[Item]:
    """Load items from Excel file (product_data.xlsx)"""
    df = pd.read_excel(excel_path)
    items = []
    
    required_columns = ['Nama', 'Harga', 'Berat', 'kategori nama']
    for col in required_columns:
        if col not in df.columns:
            raise ValueError(f"Required column '{col}' not found in Excel file")
    
    for idx, row in df.iterrows():
        # Calculate value based on weight/price ratio (can be customized)
        price = float(row['Harga'])
        weight = float(row['Berat'])
        value = (weight / max(price, 1)) * 1000  # Simple value calculation
        
        item = Item(
            id=int(row.get('no', idx)),
            category=str(row['kategori nama']) if pd.notna(row['kategori nama']) else 'Unknown',
            name=str(row['Nama']),
            price=price,
            weight=weight,
            value=value,
            store=str(row.get('Toko', '')) if 'Toko' in df.columns else ''
        )
        items.append(item)
...
def test_recommendation_system():
    ...
    # Create sample items
    sample_items = [
        Item(1, "Abon", "Abon Ayam A", 25000, 100, 80, "Store A"),
        Item(2, "Abon", "Abon Ayam B", 30000, 120, 90, "Store B"),
        Item(3, "Kripik", "Kripik Pisang A", 15000, 80, 70, "Store A"),
        Item(4, "Kripik", "Kripik Pisang B", 18000, 90, 75, "Store C"),
        Item(5, "Bakpia", "Bakpia A", 20000, 150, 85, "Store B"),
        Item(6, "Bakpia", "Bakpia B", 22000, 160, 88, "Store A"),
        Item(7, "Emping", "Emping A", 12000, 60, 65, "Store C"),
        Item(8, "Emping", "Emping B", 14000, 70, 68, "Store B"),
    ]
